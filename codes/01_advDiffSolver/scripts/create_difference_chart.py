...
def create_difference_chart(json_path=None):
    ...
    plt.plot(nX, time_differences, "o-", linewidth=2, markersize=8, 
             label=f"Differenz: {lang1.title()} - {lang2.title()}")
    plt.axhline(y=0, color='gray', linestyle='--', linewidth=1)
    
    plt.legend()
    plt.xlabel("Nx")
    plt.ylabel("Runtime Difference (s)")
    plt.title(f"Runtime Difference: {lang1.title()} - {lang2.title()}")
