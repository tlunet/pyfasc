    ...
    void setup(istream& in) {
        ...
        if (flowType == "diagonal") {
            ...
            for (int s = -nHalo; s < nHalo+1; s++) {
                // BUG: Vorzeichen falsch - Diffusion wird instabil!
                valX(s, x, y) = -cAdv[s+nHalo]/dX - viscosity*cDif[s+nHalo]/dX2;  // - statt +
                valY(s, x, y) = -cAdv[s+nHalo]/dY - viscosity*cDif[s+nHalo]/dY2;  // - statt +
            }
